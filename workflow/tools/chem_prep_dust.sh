...
if [[ -s ${DUST_EXTERNAL} ]]; then
   ...
   srun python -u "${SCRIPT}" \
              ...
              "${INTERP_WEIGHTS_DIR}" \
              "${YYYY}${MM}${DD}${HH}"
   OUTFILE_EROD=${DUST_OUTPUTDIR}/dust_inputs.erod.${MESH_NAME}.nc
   ...
   ncks -A -v erod "${OUTFILE_EROD}" "${OUTFILE_2D_Time}"

   cp "${OUTFILE_2D_Time}" "${DUST_INITFILE}"
   ncrename -v ssm,ssm_in -v sandfrac,sandfrac_in -v clayfrac,clayfrac_in -v uthres,uthres_in -v rdrag,rdrag_m_in -v erod,erod_in "${DUST_INITFILE}"
